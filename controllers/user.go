...
// Login es el controlador de sesion del login
func Login(w http.ResponseWriter, r *http.Request) {
	...
	if user.ID > 0 {
		...
		m := models.Message{
			Message: "Usuario o clave no es válido",
...
// UserCreate es el controlador para registrar un nuevo usuario
func UserCreate(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		m.Message = fmt.Sprintf("Erro al crear el registro:%s", err)
		m.Code = http.StatusBadRequest
		commons.DisplayMessage(w, m)
		return
	}
